...
const cards = [
    { name: 'All', href: '#', icon: CheckCircleIcon, total: '' },
    { name: 'Published', href: '#', icon: CheckCircleIcon, total: '' },
    { name: 'Drafted', href: '#', icon: CheckCircleIcon, total: '' },
    { name: 'Trashed', href: '#', icon: CheckCircleIcon, total: '' },
...
export default function Example() {
    ...
    useEffect(() => {
        const fetcharticles = async () => {
            const data = await axios.get("http://localhost:9999/article/")
    ...
    const group = () => {
        let status = articles.filter(article => {
            if (article.status === getStatus) {
                return article;
            }
        })
        return status;
                                        ...
                                        <div key={card.name} className="bg-white overflow-hidden shadow rounded-lg">
                                            ...
                                            <div className="bg-gray-50 px-5 py-3">
                                                <div className="text-sm">
                                                    <a href={card.href} onClick={() => setStatus(card.name)} className="font-medium text-cyan-700 hover:text-cyan-900">
                                                        View all
                                                    </a>
                                                </div>
                                            </div>
